...
var MpoolPush = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		if tos := cctx.String("smsg-hex"); tos != "" {
			msgByte, err := hex.DecodeString(strings.TrimSpace(tos))
			...
			msg, err = types.DecodeSignedMessage(msgByte)

			if err != nil {
				return fmt.Errorf("failed to get DecodeSignedMessage: %w", err)
			}

			//fmt.Printf("mbCid: %v\n", msg.Message.Cid())
			//fmt.Printf("msg.Message.From: %v\n", msg.Message.From)
			//fmt.Printf("msg.Signature.Type: %v\n", msg.Signature.Type)
			//fmt.Printf("msg.Signature.Data: %v\n", msg.Signature.Data)


			//nonce, err := api.MpoolGetNonce(ctx, msg.Message.From)
			//
			//if err != nil {
			...
			return fmt.Errorf("smsg-hex is null")
		}
