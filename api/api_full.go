...
type FullNode interface {
	...
	StateGetParentDeals(ctx context.Context, blockCid cid.Cid) ([]*StateGetParentDealsResp, error)
	StateBlockMessages(ctx context.Context, blockCid cid.Cid) (*StateBlockMessagesRes, error)
	StateGetParentReceipts(ctx context.Context, blockCid cid.Cid) ([]*types.MessageReceipt, error)
	StateMinerPartitionsNew(ctx context.Context, m address.Address, dlIdx uint64, tsk types.TipSetKey) ([]*das.Partition, error)
	StateMinerDeadlinesByMiner(context.Context, address.Address, types.TipSetKey) ([]*StateMinerDeadlinesByMinerInfo, error)

	StateGetActorsInfo(ctx context.Context, tsk types.TipSetKey) (*ActorStates, error)
	StateGetRewardLastPerEpochReward(context.Context, types.TipSetKey) (abi.TokenAmount, error)
	StatePreCommittedSectors(context.Context, address.Address, types.TipSetKey) ([]uint64, error)
	StateMinerSectorNumbers(context.Context, address.Address, types.TipSetKey) ([]uint64, error)

	WalletBalanceByTs(context.Context, address.Address,types.TipSetKey) (types.BigInt, error)

	StateReplayList(ctx context.Context, tps *types.TipSet) ([]*InvocResult, error)

	StateReplayLists(ctx context.Context,  height int64) ([]*InvocResult, error)

	StateComputeBaseFee(ctx context.Context, blockCid cid.Cid) (abi.TokenAmount, error)



	StateCallExt(context.Context,  *types.Message, types.ChainMsg, types.TipSetKey) (*InvocResult, error)

	StateReplayEx(context.Context, types.TipSetKey) ([]*InvocResults, error)


	StateGetMessageByHeightInterval(context.Context, int64 ) ([]*NMessage, error)

	StateGetBlockByHeightInterval(context.Context, int64 ) ([]*types.BlockHeaders, error)

	StateGetBlockByHeightWhere(ctx context.Context, params StateGetBlockByHeightIntervalParams ) ([]*types.BlockHeaders, error)



	StateGetMessageDealsByHeightInterval(ctx context.Context, params int64 ) ([]*StateGetParentDealsResp, error)

	StateGetMessageByHeightIntervalEx(ctx context.Context, params StateGetMessageByHeightIntervalParams) ([]*SMessage, error)



	StateGetBlockByHeight(context.Context, int64,int64 ) ([]*Blocks, error)

	StateMinerVestingFundsByHeight(context.Context, address.Address, abi.ChainEpoch,abi.ChainEpoch, types.TipSetKey) ([]map[string]interface{}, error)

	StateMessageByHeight(context.Context, abi.ChainEpoch) ([]*InvocResultExt, error)

	ClientIpfsAddGo(ctx context.Context, params ClientIpfsAddGoParams) (ClientIpfsAddGoParams, error)

	StateComputeBaseFeeTest(ctx context.Context) (abi.TokenAmount, error)

	StateReplayNoTs(context.Context, cid.Cid) (*InvocResult, error) //perm:read


	StateMarketStorageDealNoTs(context.Context, abi.DealID) (*MarketDeal, error) //perm:readc

	StateTest(context.Context ,StateTestParams) (abi.SectorQuality, error)

	StateSectorGetInfoQa(context.Context, address.Address, abi.SectorNumber, types.TipSetKey) (*StateSectorGetInfoQaResp, error) //perm:read

	StateMinerSectorsQa(context.Context, address.Address, *bitfield.BitField, types.TipSetKey) ([]*SectorOnChainInfoQa, error) //perm:read



	StateWithdrawal(ctx context.Context, params *StateWithDrawalReq) (string, error)

	StateGetMessage(context.Context, cid.Cid) (*NMessage, error)

	StateMpoolReplace(context.Context, cid.Cid) (string, error)
	...
	StateDecodeReturn(ctx context.Context, toAddr address.Address, method abi.MethodNum, params []byte) (string, error) //perm:read
...
type StateGetParentDealsResp struct {
	ClientWallet string
	CID    cid.Cid
	DealID         abi.DealID
	Proposal market.DealProposal
...
type StateBlockMessagesRes struct {
...
type ClientIpfsAddGoParams struct {
	Path string
	Key  string
	Hash string
}

type StateTestParams struct {
	Size uint64
	Duration int64
	DealWeight int64
...
type StateGetMessageByHeightIntervalParams struct {
	ToAddress []string
	FromAddress  []string
	Method []string
	CodeMethod string
	BlockIdStart       cid.Cid
	BlockIdEnd cid.Cid
}

type StateSectorGetInfoQaResp struct {
	SectorQuality abi.SectorQuality
	SectorOnChainInfo *miner.SectorOnChainInfo
}

type StateGetBlockByHeightIntervalParams struct {
	MinerAddress []string
	StartHeight int
	EndHeight int
}

type StateWithDrawalReq struct {
	To  string
	From string
	Amount   string
...
type SectorOnChainInfoQa struct {
	SectorQuality         abi.SectorQuality
...
type StateGetMessageDealsByHeightIntervalReq struct {
	Height  int64
