...
func (s *state4) LoadVestingFundList() ([]map[string]interface{}, error) {
	vfs, err := s.LoadVestingFunds(s.store)
	if err != nil {
		return nil, err
	}
	log.Infof(" VestingSize %v",len(vfs.Funds))
	var res []map[string]interface{}

	for _, fund := range vfs.Funds {
		m := make(map[string]interface{})

		m["Epoch"] = fund.Epoch
		m["Amount"] = fund.Amount
		res = append(res, m)
