...
func (cs *ChainStore) MessagesForTipsetEx(ts *types.TipSet) ([]types.ChainMsg, []types.ChainMsg,error) {
	bmsgs, err := cs.BlockMsgsForTipset(ts)
	if err != nil {
		return nil,nil, err
	}

	var blsOut []types.ChainMsg
	var secpkOut []types.ChainMsg
	for _, bm := range bmsgs {
		for _, blsm := range bm.BlsMessages {
			// blsm.VMMessage().BlockCid = bm.BlockId
			blsOut = append(blsOut, blsm)
		}

		for _, secm := range bm.SecpkMessages {
			//secm.VMMessage().BlockCid = bm.BlockId
			secpkOut = append(secpkOut, secm)
		}
	}

	return blsOut,secpkOut, nil
}
