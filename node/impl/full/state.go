...
func (a *StateAPI) StateReplay(ctx context.Context, tsk types.TipSetKey, mc cid.Cid) (*api.InvocResult, error) {
	...
	actor, err = a.StateManager.LoadActorTsk(ctx, m.To, types.EmptyTSK)
	if err != nil {
		actor, err = a.StateGetActor(ctx, m.To, types.EmptyTSK)
	}

	par, err := jsonParams(actor.Code, m.Method, m.Params)
	if err != nil {
		return nil, xerrors.Errorf("loading jsonParams %s: %w", tsk, err)
	}
